...
generate:
	...
	mockgen --destination ./serverscom/testing/kubernetes_clusters_mock.go --package=serverscom_testing --source ${GO_CLIENT_PATH}/kubernetes_clusters.go
	sed -i '' 's|github.com/serverscom/cloud-controller-manager/vendor/||' \
  		./serverscom/testing/collection_mock.go \
  		./serverscom/testing/cloud_computing_instances_mock.go \
  		./serverscom/testing/hosts_mock.go \
  		./serverscom/testing/load_balancers_mock.go \
  		./serverscom/testing/kubernetes_clusters_mock.go
